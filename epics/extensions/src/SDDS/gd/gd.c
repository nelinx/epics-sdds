...
epicsShareFunc gdImagePtr gdImageCreate (int sx, int sy)
{
  int i;
  gdImagePtr im;
...
epicsShareFunc gdImagePtr gdImageCreateTrueColor (int sx, int sy)
{
  int i;
  gdImagePtr im;
  im = (gdImage *) gdMalloc (sizeof (gdImage));
...
epicsShareFunc void gdImageDestroy (gdImagePtr im)
{
  int i;
  if (im->pixels)
    {
...
epicsShareFunc int gdImageColorAllocate (gdImagePtr im, int r, int g, int b)
{
...
epicsShareFunc void gdImageColorTransparent (gdImagePtr im, int color)
{
  if (!im->trueColor)
    {
      if (im->transparent != -1)
...
epicsShareFunc void gdImagePaletteCopy (gdImagePtr to, gdImagePtr from)
{
  int i;
  int x, y, p;
  int xlate[256];
...
epicsShareFunc void gdImageSetPixel (gdImagePtr im, int x, int y, int color)
{
  int p;
  switch (color)
    {
...
epicsShareFunc void gdImageLine (gdImagePtr im, int x1, int y1, int x2, int y2, int color)
{
...
epicsShareFunc void gdImageFilledRectangle (gdImagePtr im, int x1, int y1, int x2, int y2,
			int color)
{
  int x, y;
...
epicsShareFunc void gdImageSetBrush (gdImagePtr im, gdImagePtr brush)
{
  int i;
  im->brush = brush;
...
epicsShareFunc void gdImageInterlace (gdImagePtr im, int interlaceArg)
{
