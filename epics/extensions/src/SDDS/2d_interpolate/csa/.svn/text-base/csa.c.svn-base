...
int do_csa_2d_interpolate(specs *spec, int nin, point *pin, int *nout, point **pout, double *std)
{
  ...
  if (!(*nout)) {
    if (isnan(spec->zoom) || spec->zoom<=1.0) {
      points_generate(a->xmin - a->h, a->xmax + a->h, a->ymin - a->h, a->ymax + a->h, spec->nx, spec->ny, nout, pout);
      spec->xmin = a->xmin - a->h;
      spec->xmax = a->xmax + a->h;
      spec->ymin = a->ymin - a->h;
      spec->ymax = a->ymax + a->h;
    } else {
      double xdiff2 = (a->xmax - a->xmin) / 2.0;
      double ydiff2 = (a->ymax - a->ymin) / 2.0;
      double xav = (a->xmax + a->xmin) / 2.0;
      double yav = (a->ymax + a->ymin) / 2.0; 
      points_generate(xav - xdiff2 * spec->zoom, xav + xdiff2 * spec->zoom, yav - ydiff2 * spec->zoom, yav + ydiff2 * spec->zoom, spec->nx, spec->ny, nout, pout);
