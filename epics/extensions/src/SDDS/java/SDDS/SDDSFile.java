...
public class SDDSFile {
    ...
    public boolean isURL = false, isIS = false;
    /**
     ...
     * 
     * @param is0            InputStream object to read from
     */
    public SDDSFile(InputStream is0) {
	String os = System.getProperty("os.name");
        fileName = null;
	is = is0;
	isIS = true;
	...
	for (int i=0; parameters && i<file.parameterCount; i++){
		...
		case SDDS_LONG:
		    ...
		    break;
		case SDDS_ULONG:
		    if (!setParameter(i, (long)(((Long)object).longValue() & 0xffffffffL), page))
			return false;
		    break;
		case SDDS_SHORT:
		    ...
		    break;
		case SDDS_USHORT:
		    if (!setParameter(i, (int)(((Integer)object).intValue() & 0xffff), page))
			return false;
		    break;
		case SDDS_STRING:
		case SDDS_CHARACTER:
	...
	for (int i=0; arrays && i<file.arrayCount; i++){
		...
		case SDDS_ULONG:
			...
			long[] values = new long[size];
			for (int j=0;j<size;j++)
			    values[j] = (long)(((Long)objects[j]).longValue() & 0xffffffffL);
			if (!setArray(i, values, page))
			    return false;
			break;
		    }
		...
		case SDDS_USHORT:
			...
			for (int j=0;j<size;j++)
			    values[j] = (int)(((Integer)objects[j]).intValue() & 0xffff);
			if (!setArray(i, values, page))
			    return false;
			break;
		    }
	...
	if (is == null && fileName == null) {
		...
		try {
		    ...
		    setError((isURL?"URL":"input file") + " does not exist (openInputFile)");
		    e.printStackTrace();
		    return false;
		}
	    }
	...
	try {
		...
		return false;
            ...
            if ((layoutVersion != 1) && (layoutVersion != 2) && (layoutVersion != 3)) {
	...
	outer:
		...
		case 3:
		    ...
		    if (token.equals("column_major_order")){
			c = st.nextToken();
			if ((c != StreamTokenizer.TT_WORD) && (c != quoteChar))
			    break outer;
			try {
		...
		while (true) {
		  try {
			...
			case SDDS_SHORT:
			    ...
			    break;
			case SDDS_USHORT:
			    setParameter(i,(int)(dis.readShort() & 0xffff),page);
			    break;
			case SDDS_LONG:
			    setParameter(i,dis.readInt(),page);
			    break;
			case SDDS_ULONG:
			    setParameter(i,(long)(dis.readInt() & 0xffffffffL),page);
			    break;
			case SDDS_FLOAT:
			...
			for (j=0;j<arraySize;j++){
				...
				break;
			    case SDDS_USHORT:
				arrDoubleValues[i][j] = (double)((int)(dis.readShort() & 0xffff));
				break;
			    case SDDS_LONG:
				arrDoubleValues[i][j] = (double)dis.readInt();
				break;
			    case SDDS_ULONG:
				arrDoubleValues[i][j] = (double)((long)(dis.readInt() & 0xffffffffL));
				break;
			    case SDDS_FLOAT:
				...
				break;
                    ...
                    if (columnMajorOrder) {
                        for (i=0;i<columnCount;i++) {
                            ...
                            case SDDS_USHORT:
                                for (row=0;row<rows;row++)
                                    doubleValues[i][row] = (double)((int)(dis.readShort() & 0xffff));
                                break;
                            case SDDS_LONG:
                                for (row=0;row<rows;row++)
                            ...
                            case SDDS_ULONG:
                                for (row=0;row<rows;row++)
                                    doubleValues[i][row] = (double)((long)(dis.readInt() & 0xffffffffL));
                                break;
                            case SDDS_FLOAT:
                                for (row=0;row<rows;row++)
		...
		try {
		  ...
		  return false;
			...
			case SDDS_SHORT:
			    ...
			    break;
			case SDDS_USHORT:
			    setParameter(i,(int)(ledis.readShort() & 0xffff),page);
			    break;
			case SDDS_LONG:
			    setParameter(i,ledis.readInt(),page);
			    break;
			case SDDS_ULONG:
			    setParameter(i,(long)(ledis.readInt() & 0xffffffffL),page);
			    break;
			case SDDS_FLOAT:
			...
			for (j=0;j<arraySize;j++){
				...
				break;
			    case SDDS_USHORT:
				arrDoubleValues[i][j] = (double)((int)(ledis.readShort() & 0xffff));
				break;
			    case SDDS_LONG:
				arrDoubleValues[i][j] = (double)ledis.readInt();
				break;
			    case SDDS_ULONG:
				arrDoubleValues[i][j] = (double)((long)(ledis.readInt() & 0xffffffffL));
				break;
			    case SDDS_FLOAT:
				...
				break;
                    ...
                    if (columnMajorOrder) {
                        for (i=0;i<columnCount;i++) {
                            ...
                            case SDDS_USHORT:
                                for (row=0;row<rows;row++)
                                    doubleValues[i][row] = (double)((int)(ledis.readShort() & 0xffff));
                                break;
                            case SDDS_LONG:
                                for (row=0;row<rows;row++)
                            ...
                            case SDDS_ULONG:
                                for (row=0;row<rows;row++)
                                    doubleValues[i][row] = (double)((long)(ledis.readInt() & 0xffffffffL));
                                break;
                            case SDDS_FLOAT:
                                for (row=0;row<rows;row++)
     ...
     *                         <li><code>4</code> for ulong</li>
     *                         <li><code>5</code> for short</li>
     *                         <li><code>6</code> for ushort</li>
     *                         <li><code>7</code> for string</li>
     *                         <li><code>8</code> for character</li>
     *                         </ul>
     ...
     *                         <li><code>"ulong"</code> for ulong</li>
     *                         <li><code>"short"</code> for short</li>
     *                         <li><code>"ushort"</code> for ushort</li>
