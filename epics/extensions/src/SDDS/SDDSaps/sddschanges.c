...
#define SET_MAJOR_ORDER 7
#define N_OPTIONS 8

char *option[N_OPTIONS] = {
  "copy", "changesin", "pass", "baseline", "pipe", "parallelpages", "keepempties", "majorOrder",
} ;
char *optionSuffix[3] = {
  "", "ChangeIn", "", 
} ;
...
long setupOutputFile(SDDS_DATASET *outSet, char *output, SDDS_DATASET *inSet, 
                     CHANGE_DEFINITION *change, long changes, short columnMajorOrder);
...
[-baseline=<filename> [-parallelPages]] [-keepEmpties] [-majorOrder=row|column] \n\n\
...
majorOrder Specify write output file in row or column major. \n\n\
