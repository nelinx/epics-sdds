...
#define SET_MAJOR_ORDER 32
#define SET_EVALUATE 33
#define N_OPTIONS 34
  ...
  char *option[N_OPTIONS] = {
    ...
    "description", "fclip", "timeFilter", "majorOrder", "evaluate",
  } ;
...
" [-evaluate={column|parameter},<new-name>,<expression-name>[,<definition-entries>]] \n",
...
" [-description=[text=<string>][,contents=<string>]] [-majorOrder=row|column]\n",
" \n",
...
int main(int argc, char **argv)
{
    ...
    unsigned long flags, majorOrderFlags;
	...
	case SET_EVALUATE:
	  ...
	  evaluate_definition = trealloc(evaluate_definition, sizeof(*evaluate_definition)*(evaluate_definitions+1));
	  if (!(evaluate_definition[evaluate_definitions]
		=process_new_evaluate_definition(s_arg[i_arg].list+1, s_arg[i_arg].n_items-1)))
	    SDDS_Bomb("invalid -definition syntax");
	...
	index = evaluate_ptr->is_parameter ?
	  SDDS_GetParameterIndex(&SDDS_output, evaluate_ptr->name) :
	    SDDS_GetColumnIndex(&SDDS_output, evaluate_ptr->name) ;
	if (index>=0) {
	...
	if ((index = evaluate_ptr->is_parameter ?
             SDDS_GetParameterIndex(&SDDS_output, evaluate_ptr->source) :
             SDDS_GetColumnIndex(&SDDS_output, evaluate_ptr->source))<0) {
        ...
        if ((evaluate_ptr->is_parameter?SDDS_GetParameterType(&SDDS_output, index):
             SDDS_GetColumnType(&SDDS_output, index))!=SDDS_STRING) {
        ...
        if (!(evaluate_ptr->is_parameter ?
              SDDS_ProcessParameterString(&SDDS_output, evaluate_ptr->text, SDDS_WRITEONLY_DEFINITION):
              SDDS_ProcessColumnString(&SDDS_output, evaluate_ptr->text, SDDS_WRITEONLY_DEFINITION))) {
          SDDS_PrintErrors(stderr, SDDS_VERBOSE_PrintErrors);
          exit(1);
        }
	break;
	...
	case IS_EVALUATE_DEFINITION:
	  ...
	  fprintf(stderr, "evaluating to %s %s from %s\n",
		  evaluate_ptr->is_parameter?"parameter":"column",
		  evaluate_ptr->name, evaluate_ptr->source);
	  break;
	case IS_EDIT_DEFINITION:
	  edit_ptr = (EDIT_DEFINITION*)definition[i].structure;
...
long SDDS_EvaluateParameter(SDDS_DATASET *SDDS_dataset, EVALUATE_DEFINITION *definition)
{
  EVALUATE_DEFINITION *evaluate_ptr;
  double value;
  char **equation;
  long j, parameter;
  ...
  case SDDS_USHORT:
    ...
    break;
  case SDDS_LONG:
    SDDS_SetParameters(SDDS_dataset, SDDS_SET_BY_INDEX|SDDS_PASS_BY_VALUE, parameter, (long)value, -1);
    break;
  case SDDS_ULONG:
    SDDS_SetParameters(SDDS_dataset, SDDS_SET_BY_INDEX|SDDS_PASS_BY_VALUE, parameter, (unsigned long)value, -1);
    break;
  case SDDS_FLOAT:
