...
[-latexFormat[=longtable][,booktable][,label=<string>][,caption=<string>][,group=<columnName>][,translate=<filename>] [-noWarnings]\n\
-spreadsheet=csv is the simple way of -spreadsheet=nolabels,quote=\",delimiter=\\, -notitle \n\
Translation file for LaTeX mode has columns OldString and NewString, and can be used to translate symbols and units.\n\n\
Program by Michael Borland. (This is version 9, September 2013.)\n";

#define PAGINATION_ON       0x0001U
#define PAGINATION_NOTITLE  0x0002U
#define PAGINATION_NOLABELS 0x0004U

#define LATEX_FORMAT    0x0001U
#define LATEX_LONGTABLE 0x0002U
#define LATEX_BOOKTABLE 0x0004U
#define LATEX_LABEL     0x0008U
#define LATEX_CAPTION   0x0010U
#define LATEX_GROUP     0x0020U
#define LATEX_TRANSLATE 0x0040U
...
long makeColumnHeaders(char ***header, long *fieldWidth, char *name, char *editLabel, char *units, char **format,
                       unsigned long spreadsheetFlags, unsigned long latexFormat, htab *translationTable);
...
long processPrintColumns(PRINT_COLUMN **printRequestPtr, long printRequests, SDDS_DATASET *inTable, long noWarnings,
                         unsigned long spreadsheetFlags, long csv, unsigned long latexformat, htab *translationTable);
...
void doPrintColumns(SDDS_DATASET *inTable, PRINT_COLUMN *printColumn, long printColumns, 
                    ...
                    char *spreadsheetQuoteMark, long latexFormat, char *latexTitle, char *latexLabel, char *latexGroupColumn,
                    PAGINATION *pagination, char *title, long noLabels);
...
htab *readLatexTranslationTable(char *latexTranslationFile);
...
int main(int argc, char **argv)
{
    ...
    char *latexLabel, *latexCaption, *latexGroup, *latexTranslationFile;
    htab *latexTranslationTable;
    ...
    if (latexFormat&LATEX_TRANSLATE) 
      latexTranslationTable = readLatexTranslationTable(latexTranslationFile);
    ...
    printColumns = processPrintColumns(&printColumn, printColumns, &SDDS_dataset, noWarnings, spreadsheetFlags, CSV, latexFormat, latexTranslationTable);
...
/*        doPrintArrays(&SDDS_dataset, printArray, printArrays, width, fpOut, &pagination); */
        doPrintColumns(&SDDS_dataset, printColumn, printColumns, width, fpOut,
                       ...
                       latexFormat, latexCaption, latexLabel, latexGroup, &pagination, title, noLabels);
        if (bufferLines) {
...
long processPrintColumns(PRINT_COLUMN **printRequestPtr, long printRequests, SDDS_DATASET *inTable, long noWarnings,
                         unsigned long spreadsheetFlags, long csv, unsigned long latexFormat, htab *latexTranslationTable)
{
          ...
          if (!columnUsed[index]) {
            ...
            printColumn[printColumns].headers = 
              makeColumnHeaders(&printColumn[printColumns].header, 
                                ...
                                spreadsheetFlags,
                                latexFormat, latexTranslationTable);
#if defined(DEBUG)
...
long makeColumnHeaders(char ***header, long *fieldWidth, char *name, char *editLabel, char *units, char **format,
                       unsigned long spreadsheetHeaders, unsigned long latexFormat, htab *latexTranslationTable)
{
  ...
  if (latexFormat&LATEX_TRANSLATE) {
    name  = findTranslation(latexTranslationTable, name);
    units = findTranslation(latexTranslationTable, units);
  }
                    ...
                    char *title, long noLabels)
{
  ...
  long nGroups, maxGroupLength, groupLength;
  if (!printColumns)
    return;
  ...
  if (!(spreadsheetFlags&SPREADSHEET_ON) && !noLabels)
    printColumnHeaders(fpOut, printColumn, printColumns, width, pagination, latexFormat, latexTitle?latexTitle:title);
  ...
  if (latexFormat&LATEX_GROUP) {
    ...
    if (groupLength>maxGroupLength) 
      maxGroupLength = groupLength;
    if (latexFormat)
      fprintf(fpOut, "%% nGroups = %ld, maxGroupLength = %ld\n", nGroups, maxGroupLength);
  }
...
htab *readLatexTranslationTable(char *latexTranslationFile)
{
  ...
  long rows;
#if defined(_WIN32)
  SDDS_Bomb("The latex option in sddsprintout is not available on Windows");
#else
  ht = hcreate(12);
  if (!SDDS_InitializeInput(&SDDSin, latexTranslationFile) ||
