...
#define CLO_MAJOR_ORDER 6
#define CLO_OPTIONS 7

char *option[CLO_OPTIONS] = {
  "order","pipe","belowrange","aboverange","data","verbose", "majorOrder",
};
...
static char *USAGE1 = "sddsinterpset [<input>] [<output>] [-pipe=[input][,output]]\n\
 ...
 [-data=fileColumn=<colName>,interpolate=<colName>,functionof=<colName>, \n\
     column=<colName> | atValue=<value>] [-majorOrder=row|column] \n\
...
-data option: \n\
  ...
  -column     -- Gives the name of a column in <input>.  The primary output \n\
                   is I(C). \n\
  -majorOrder -- Specifies writing the output files in row or column major order.\n";
...
int main(int argc, char **argv)
{
  ...
  unsigned long pipeFlags, dummyFlags, interpCode, majorOrderFlag;
