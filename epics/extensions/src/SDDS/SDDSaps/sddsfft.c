 ...
 * based on mpl-format fft program
 ...
 Revision 1.38  2013/04/19 shang
 removed two wrong and useless statements: "flags != FL_FULLOUTPUT_FOLDED;" and "complexInputFlags != FL_COMPLEXINPUT_FOLDED;" 
...
char *option[N_OPTIONS] = {
    ...
    "nowarnings", "complexinput", "inverse", "majorOrder",
    };
...
[-sampleInterval=<number>] [-normalize] [-fullOutput[=unfolded|folded],unwrapLimit=<value>]\n \
[-psdOutput[=plain][,{integrated|rintegrated[=<cutoff>]}] [-inverse]\n \
[-padwithzeroes[=exponent] | -truncate] [-suppressaverage] [-noWarnings] [-majorOder=row|column] \n\n\
...
static char *USAGE2="-window          Requests windowing of data prior to analysis.  The default\n\
                 window is Hanning (i.e., -window is equivalent to -window=hanning).\n\
                 The \"none\" option is provided but strictly unnecessary; one may simply\n\
                 leave off the window option altogether to perform analysis without windowing.\n\
                 If the \"correct\" qualifier is given, the signal is also multiplied by a correction\n\
                 factor to ensure that the integrated PSD is unchanged by the window.\n\
...
-fullOutput      Requests output of the real and imaginary parts of the FFT.\n\
                 ...
                 when unwrapLimit is given, the unwrapping phase will be computed and output to UnwrappingArg<signal> \n\
                 column. if unwrapLimit is greater than zero, only unwrapping phase where the relative magnitude is\n\
                 greater than this limit, otherwise, unwrapping everywhere.\n\
-psdOutput       Requests output of the Power Spectral Density.  By default, the\n\
                 ...
                 gives both. <cutoff> provides the cutoff frequency for reverse-integrated PSD.\n\
...
long process_data(SDDS_DATASET *SDDSout, SDDS_DATASET *SDDSin, double *tdata, long rows, long rowsToUse,
                  ...
                  long correctWindowEffects, long inverse, double rintegCutOffFreq, double unwrapLimit);
...
long create_fft_columns(SDDS_DATASET *SDDSout, SDDS_DATASET *SDDSin, char *origName,
			char *indepName, char *freqUnits, long full_output, unsigned long psd_output,
                        long complexInput, long inverse, long unwrap_phase);
...
int main(int argc, char **argv)
{
  ...
  char *input, *output;
  long sampleInterval, i, rows, readCode, rowsToUse, noWarnings, complexInput, inverse, spectrumFoldParExist=0;
      ...
      case SET_FULLOUTPUT:
        ...
        if (scanned[iArg].n_items>=2) {
          ...
          if (!scanItemList(&fullOutputFlags, scanned[iArg].list+1, &scanned[iArg].n_items, 0,
                            ...
                            "unwrapLimit", SDDS_DOUBLE, &unwrapLimit, 0, FL_UNWRAP_PHASE,
                            NULL))
	...
	if (scanned[iArg].n_items -=1) {
	  unsigned long tmpFlags;
          if (strchr(scanned[iArg].list[1], '=')<=0) {
            ...
            if (!scanItemList(&tmpFlags, scanned[iArg].list+1, &scanned[iArg].n_items, 0,
                              "integrated", -1, NULL, 0, FL_PSDINTEGOUTPUT,
                              "rintegrated", SDDS_DOUBLE, &rintegCutOffFreq, 0, FL_PSDRINTEGOUTPUT,
                              "plain", -1, NULL, 0, FL_PSDOUTPUT,
                              NULL))
              SDDS_Bomb("invalid -psdOutput syntax");
          }
  ...
  for (i=0; i<depenQuantities; i++) {
    if (!complexInput)
      create_fft_columns(&SDDSout, &SDDSin, depenQuantity[i], indepQuantity, freqUnits,
                         flags&FL_FULLOUTPUT, 
                         flags&(FL_PSDOUTPUT+FL_PSDINTEGOUTPUT+FL_PSDRINTEGOUTPUT), 0, inverse, flags&FL_UNWRAP_PHASE);
    else
      create_fft_columns(&SDDSout, &SDDSin, realQuan[i], indepQuantity, freqUnits,
                         flags&FL_FULLOUTPUT, 
                         flags&(FL_PSDOUTPUT+FL_PSDINTEGOUTPUT+FL_PSDRINTEGOUTPUT), 1, inverse, flags&FL_UNWRAP_PHASE);
  }
  ...
  while ((readCode=SDDS_ReadPage(&SDDSin))>0) {
	...
	if (!process_data(&SDDSout, &SDDSin, tdata, rows, rowsToUse, 
			  ...
			  flags|(i==0?FL_MAKEFREQDATA:0), windowType, sampleInterval,
                          correctWindowEffects, inverse, rintegCutOffFreq, unwrapLimit)) {
                  ...
                  long correctWindowEffects, long inverse, double rintegCutOffFreq, double unwrapLimit)
{
  ...
  double *real, *imag, *magData, *arg=NULL, *real_imag, *data, *psd=NULL, *psdInteg=NULL, *unwrapArg=NULL, phase_correction=0;
