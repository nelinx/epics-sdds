...
#define SET_MAJOR_ORDER 5
#define N_OPTIONS 6

static char *optionName[N_OPTIONS] = {
  "column", "parameter", "pipe", "outofbounds", "constant", "majorOrder",
};
...
void setupOutputFile(SDDS_DATASET *outSet, char *output, SDDS_DATASET *inSet, 
                     char *input, DEREF_REQUEST *deref, long derefs, short columnMajorOrder);
...
int main(int argc, char **argv)
{
    ...
    setupOutputFile(&outSet, output, &inSet, input, derefRequest, 
                    derefRequests, columnMajorOrder);
