...
char *option[N_OPTIONS] = {
  ...
  "nowarnings", "ifis", "pipe", "fillin", "rename","editnames", "wildmatch", "majorOrder",
} ;
...
[-equate=<column-name>[=<column-name>]] [-majorOrder=row|column] \n\n\
            ...
            for (i1=0; i1<rows1; i1++) {
              if (firstRun) {
                if (!SDDS_CopyRowDirect(&SDDS_output, i1, &SDDS_1, i1)) {
                  sprintf(s, "Problem copying row %d of first data set", i1);
                  SDDS_SetError(s);
                  SDDS_PrintErrors(stderr, SDDS_VERBOSE_PrintErrors|SDDS_EXIT_PrintErrors);
                }
              }
              ...
              if ((&SDDS_output)->row_flag[i1]) {
                ...
                if (matched) {
                  ...
                  if (!fillIn && !SDDS_AssertRowFlags(&SDDS_output, SDDS_INDEX_LIMITS, i1, i1, (int32_t)0)) 
                    SDDS_PrintErrors(stderr, SDDS_VERBOSE_PrintErrors|SDDS_EXIT_PrintErrors);
                  if (warnings)
                    fprintf(stderr, "warning: no match for row %d (%s = \"%s\")\n", i3,
              ...
              if ((&SDDS_output)->row_flag[i1]) {
                if ((i2 = FindMatchingKeyGroup(keyGroup, keyGroups, SDDS_DOUBLE, value1+i3, reuse))>=0) {
                  ...
                  if (!fillIn && !SDDS_AssertRowFlags(&SDDS_output, SDDS_INDEX_LIMITS, i1, i1, (int32_t)0)) 
                    SDDS_PrintErrors(stderr, SDDS_VERBOSE_PrintErrors|SDDS_EXIT_PrintErrors);
                  if (warnings)
                    fprintf(stderr, "warning: no equal for row %d (%s = %g)\n", i3,
		  ...
		  if (fillIn) {
		    if (!SDDS_CopyRowDirect(&SDDS_output, i1, &SDDS_1, i1)) {
		      ...
		      SDDS_PrintErrors(stderr, SDDS_VERBOSE_PrintErrors|SDDS_EXIT_PrintErrors);
                  ...
                  if (warnings)
                    fprintf(stderr, "warning: no row in file 2 for row %d in file 1\n",i1);
                  continue;
                } else
                  i2 = rows2-1;
              }
