...
static PyObject* sddsdata_SetColumn( PyObject* self, PyObject* args )
{
  ...
  case SDDS_ULONG:
    ...
    for (i=0;i<rows;i++) {
      temp = PyList_GetItem(v, i);
      if (PyLong_Check(temp))
	((uint32_t*)data)[i] = (uint32_t)PyLong_AsLong(temp);
      else
	((uint32_t*)data)[i] = (uint32_t)PyInt_AsLong(temp);
    }
    break;
  case SDDS_FLOAT:
...
static PyObject* sddsdata_GetColumn( PyObject* self, PyObject* args )
{
  ...
  case SDDS_LONG:
    for (i=0;i < rows;i++) {
      PyList_SetItem(v, i, PyLong_FromLong(((int32_t*)columnValue)[i]));
    }
    free((int32_t*)columnValue);
    break;
  case SDDS_ULONG:
    for (i=0;i < rows;i++) {
      PyList_SetItem(v, i, PyLong_FromLong(((uint32_t*)columnValue)[i]));
    }
...
static PyObject* sddsdata_GetParameter( PyObject* self, PyObject* args ) 
{
  ...
  case SDDS_USHORT:
    ...
    break;
  case SDDS_LONG:
    v = PyLong_FromLong(((int32_t*)parameterValue)[0]);
