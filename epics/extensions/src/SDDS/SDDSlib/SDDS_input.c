...
int32_t SDDS_GetLZMANamelist(char *buffer, int32_t buflen, struct lzmafile* lzmafp)
{
  char *ptr, *flag, *buffer0;
  /*  char *ptr1*/
  int32_t n;
  ...
  while ((flag=fgetsLZMASkipComments(buffer, buflen, lzmafp, '!'))) {
    if ((ptr=strchr(buffer, '&')) && !SDDS_IsQuoted(buffer, ptr, '"'))
      break;
  }
  if (!flag)
...
int32_t SDDS_LZMAReadLayout(SDDS_DATASET *SDDS_dataset, struct lzmafile* lzmafp)
{
  ...
  if (depth==0) {
    ...
    SDDS_dataset->layout.layout_written = 1; /* it is already in the file */
    if (!lzma_gets(s, SDDS_MAXLINE, lzmafp)) {
      lzma_close(lzmafp);
  ...
  while (SDDS_GetLZMANamelist(s, SDDS_MAXLINE, lzmafp)) {
#if DEBUG
    strcpy(buffer, s);
#endif
    groupName = s+1;
  ...
  /* on recursive calls, it's okay to hit EOF */
  if ((lzma_eof(lzmafp) && depth!=0) || data_command_seen)
    return(1);
  return(0);
}
