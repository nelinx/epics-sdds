...
int32_t SDDS_LZMAWriteTypedValue(void *data, int32_t index, int32_t type, char *format, struct lzmafile* lzmafp)
{
  char c, *s;
  short hasWhitespace;
...
int32_t SDDS_LZMAWriteAsciiParameters(SDDS_DATASET *SDDS_dataset, struct lzmafile* lzmafp)
{
  ...
  for (i=0; i<layout->n_parameters; i++) {
    if (layout->parameter_definition[i].fixed_value)
      continue;
    if (!SDDS_LZMAWriteTypedValue(SDDS_dataset->parameter[i], 0, layout->parameter_definition[i].type,
                                  NULL, lzmafp)) {
...
int32_t SDDS_LZMAWriteAsciiRow(SDDS_DATASET *SDDS_dataset, int32_t row, struct lzmafile* lzmafp)
{
  int32_t i, n_per_line, line, newline_needed;
  /*  int32_t  embedded_quotes_present;*/
  SDDS_LAYOUT *layout;
  ...
  newline_needed = 0;
  for (i=0; i<layout->n_columns; i++) {
    if (!SDDS_LZMAWriteTypedValue(SDDS_dataset->data[i], row, layout->column_definition[i].type,
                              NULL, lzmafp)) {
  ...
  if (SDDS_dataset->layout.gzipFile) {
    ...
    if (SDDS_dataset->layout.lzmaFile) {
      ...
      for (i=0; i<layout->n_parameters; i++) {
        ...
        bigBuffer[0] = 0;
        if (!layout->parameter_definition[i].fixed_value) {
          if (!fgetsLZMASkipCommentsResize(&bigBuffer, &bigBufferSize, lzmafp, '!')) {
            if (first_read)
              return(-1);
            SDDS_SetError("Unable to read parameters--data ends prematurely (SDDS_ReadAsciiParameters)");
            return(0);
...
#endif
      if (SDDS_dataset->layout.lzmaFile) {
        if (!fgetsLZMASkipCommentsResize(&bigBuffer, &bigBufferSize, lzmafp, '!') || SDDS_StringIsBlank(bigBuffer)) {
          if (i==0)
            return(-1);
          SDDS_SetError("Unable to read array--dimensions missing (SDDS_ReadAsciiArrays)");
          return(0);
...
int32_t SDDS_ReadAsciiPage(SDDS_DATASET *SDDS_dataset, int32_t sparse_interval, int32_t sparse_offset)
{
  ...
  if (SDDS_dataset->layout.gzipFile) {
    ...
    if (SDDS_dataset->layout.lzmaFile) {
      if (SDDS_dataset->page_number==0)
        for (i=0; i<SDDS_dataset->layout.data_mode.additional_header_lines; i++) {
          if (!fgetsLZMASkipComments(s, SDDS_MAXLINE, lzmafp, '!'))
            return( SDDS_dataset->page_number= -1 );  /* indicates end of data */
        }
    } else {
      if (SDDS_dataset->page_number==0)
...
#endif
            if (SDDS_dataset->layout.lzmaFile) {
             if (!(dataRead=fgetsLZMASkipCommentsResize(&bigBuffer, &bigBufferSize, lzmafp, '!')) || 
                  SDDS_StringIsBlank(bigBuffer)) {
                SDDS_dataset->n_rows = j;
                if (no_row_counts) {
                  /* legitmate end of data set */
