...
int32_t SDDS_LZMABufferedRead(void *target, size_t targetSize, struct lzmafile* lzmafp, SDDS_FILEBUFFER *fBuffer)
{
  if (!fBuffer->bufferSize) {
  ...
  if ((fBuffer->bytesLeft-=targetSize)>=0) {
    ...
    if (fBuffer->bufferSize < bytesNeeded) {
      /* just read what is needed directly into user's memory or seek */
      if (!target)
        return !lzma_seek(lzmafp, bytesNeeded, SEEK_CUR);
      else
        return lzma_read(lzmafp, (char*)target+offset, bytesNeeded)==bytesNeeded;
    }

    if ((fBuffer->bytesLeft = lzma_read(lzmafp, fBuffer->data, fBuffer->bufferSize))<bytesNeeded)
      return 0;
    if (target)
      memcpy((char*)target+offset, (char*)fBuffer->data, bytesNeeded);
    fBuffer->data += bytesNeeded;
...
int32_t SDDS_LZMABufferedWrite(void *target, size_t targetSize, struct lzmafile *lzmafp, SDDS_FILEBUFFER *fBuffer)
{
  if (!fBuffer->bufferSize) {
    SDDS_SetError("You must presently have a nonzero file buffer to use zLib (reading/writing .gz files");
    return 0;
  ...
  if ((fBuffer->bytesLeft-=targetSize)>=0) {
    ...
    lastLeft = (fBuffer->bytesLeft += targetSize);  
    memcpy((char*)fBuffer->data, (char*)target, fBuffer->bytesLeft);
    if (lzma_write(lzmafp, fBuffer->buffer, fBuffer->bufferSize)!=fBuffer->bufferSize)
      return 0;
...
int32_t SDDS_lzmaseek(struct lzmafile* lzmafp, int32_t offset, int32_t dir)
{
  int32_t try;
#if defined(vxWorks)
  struct timespec rqtp;
  ...
  if (SDDS_dataset->layout.gzipFile) {
    ...
    if (SDDS_dataset->layout.lzmaFile) {
      ...
      for (i=0; i<layout->n_parameters; i++) {
        ...
        else if (!SDDS_LZMABufferedWrite(SDDS_dataset->parameter[i], 
                                     SDDS_type_size[layout->parameter_definition[i].type-1],
                                     lzmafp, fBuffer)) {
          SDDS_SetError("Unable to write parameters--failure writing value (SDDS_WriteBinaryParameters)");
          return(0);
  ...
  if (SDDS_dataset->layout.gzipFile) {
    ...
    if (SDDS_dataset->layout.gzipFile) {
      ...
      for (i=0; i<layout->n_arrays; i++) {
        ...
        if (!SDDS_LZMABufferedWrite(SDDS_dataset->array[i].dimension, 
                                sizeof(*(SDDS_dataset->array)[i].dimension)*
                                layout->array_definition[i].dimensions, 
                                lzmafp, fBuffer)) {
          SDDS_SetError("Unable to write arrays--failure writing dimensions (SDDS_WriteBinaryArrays)");
        ...
        else if (!SDDS_LZMABufferedWrite(SDDS_dataset->array[i].data, 
                                     SDDS_type_size[layout->array_definition[i].type-1]* 
                                     SDDS_dataset->array[i].elements, 
                                     lzmafp, fBuffer)) {
          SDDS_SetError("Unable to write arrays--failure writing values (SDDS_WriteBinaryArrays)");
	...
	for (row=0; row<SDDS_dataset->n_rows; row++) {
	  if (SDDS_dataset->row_flag[row] && !SDDS_GZipWriteBinaryString(*((char**)SDDS_dataset->data[i]+row), gzfp, fBuffer)) {
        ...
        if (rows==SDDS_dataset->n_rows) {
          if (!SDDS_GZipBufferedWrite(SDDS_dataset->data[i], 
                                  size * rows,
                                  gzfp, fBuffer)) {
    ...
    if (SDDS_dataset->layout.lzmaFile) {
      ...
      for (i=0; i<layout->n_columns; i++) {
        ...
        if (layout->column_definition[i].type==SDDS_STRING) {
          ...
          if (rows==SDDS_dataset->n_rows) {
            if (!SDDS_LZMABufferedWrite(SDDS_dataset->data[i], 
                                    size * rows,
                                    lzmafp, fBuffer)) {
      ...
      for (i=0; i<layout->n_columns; i++) {
        ...
        if (layout->column_definition[i].type==SDDS_STRING) {
          ...
          if (rows==SDDS_dataset->n_rows) {
            if (!SDDS_BufferedWrite(SDDS_dataset->data[i], 
                                    size * rows,
                                    fp, fBuffer)) {
              SDDS_SetError("Unable to write columns--failure writing values (SDDS_WriteNonNativeBinaryColumns)");
              return(0);
  ...
  if (SDDS_dataset->layout.gzipFile) {
	...
	for (row=0; row<SDDS_dataset->n_rows; row++) {
	  if (SDDS_dataset->row_flag[row] && !SDDS_GZipWriteNonNativeBinaryString(*((char**)SDDS_dataset->data[i]+row), gzfp, fBuffer)) {
        ...
        if (rows==SDDS_dataset->n_rows) {
          if (!SDDS_GZipBufferedWrite((char*)SDDS_dataset->data[i], 
                                  size * rows,
                                  gzfp, fBuffer)) {
            SDDS_SetError("Unable to write columns--failure writing values (SDDS_WriteNonNativeBinaryColumns)");
            return(0);
...
int32_t SDDS_ReadBinaryPage(SDDS_DATASET *SDDS_dataset, int32_t sparse_interval, int32_t sparse_offset)
{
  ...
  if (SDDS_dataset->layout.gzipFile) {
     ...
     */
    if (SDDS_dataset->layout.lzmaFile) {
      if (!SDDS_LZMABufferedRead(&n_rows, sizeof(n_rows), lzmafp, &SDDS_dataset->fBuffer)) {
        if (lzma_eof(lzmafp))
          return(SDDS_dataset->page_number=-1);
        SDDS_SetError("Unable to read page--failure reading number of rows (SDDS_ReadBinaryPage)");
        return(0);
  ...
  if (SDDS_dataset->layout.byteOrderDeclared == 0) {
    if (n_rows>10000000) {
      SDDS_SetError("Unable to read page--endian byte order not declared and suspected to be non-native. (SDDS_ReadBinaryPage)");
      return(0);
    }
  }
  ...
  if ((sparse_interval<=0) || (SDDS_dataset->layout.data_mode.column_major))
    sparse_interval = 1;
  if ((sparse_offset<0) || (SDDS_dataset->layout.data_mode.column_major))
    sparse_offset = 0;
  ...
  if (SDDS_dataset->layout.data_mode.column_major) {
    ...
    if (!SDDS_ReadBinaryColumns(SDDS_dataset)) {
      SDDS_SetError("Unable to read page--column reading error (SDDS_ReadBinaryPage)");
      return(0);
    }
...
char *SDDS_ReadLZMABinaryString(struct lzmafile *lzmafp, SDDS_FILEBUFFER *fBuffer, int32_t skip)
{
  ...
  if (!SDDS_LZMABufferedRead(&length, sizeof(length), lzmafp, fBuffer) || length<0)
    return(0);
  if (!(string=SDDS_Malloc(sizeof(*string)*(length+1))))
    return(NULL);
  if (length && !SDDS_LZMABufferedRead(skip?NULL:string, sizeof(*string)*length, lzmafp, fBuffer))
    return(NULL);
  string[length] = 0;
  return(string);
}
    ...
    if (SDDS_dataset->layout.lzmaFile) {
      ...
      for (i=0; i<layout->n_columns; i++) {
        ...
        if ((type=layout->column_definition[i].type)==SDDS_STRING) {
          ...
          size = SDDS_type_size[type-1];
          if (!SDDS_LZMABufferedRead(skip?NULL:(char*)SDDS_dataset->data[i]+row*size, size, lzmafp, 
                                 fBuffer)) {
            SDDS_SetError("Unable to read row--failure reading value (SDDS_ReadBinaryRow)");
            return(0);
          }
...
#endif
        if (SDDS_dataset->layout.lzmaFile) {
          if (!SDDS_LZMABufferedRead(SDDS_dataset->parameter[i], SDDS_type_size[layout->parameter_definition[i].type-1], 
                                 lzmafp, fBuffer)) {
            SDDS_SetError("Unable to read parameters--failure reading value (SDDS_ReadBinaryParameters)");
            return(0);
          }
...
#endif
      if (SDDS_dataset->layout.lzmaFile) {
        if (!SDDS_LZMABufferedRead(array->dimension, sizeof(*array->dimension)*array->definition->dimensions, 
                               lzmafp, fBuffer)) {
          SDDS_SetError("Unable to read arrays--failure reading dimensions (SDDS_ReadBinaryArrays)");
          return(0);
        }
...
#endif
        if (SDDS_dataset->layout.lzmaFile) {
          if (!SDDS_LZMABufferedRead(array->data, 
                                     SDDS_type_size[array->definition->type-1]*array->elements, 
                                     lzmafp, fBuffer)) {
            SDDS_SetError("Unable to read arrays--failure reading values (SDDS_ReadBinaryArrays)");
            return(0);
...
#if defined(zLib)
      if (SDDS_dataset->layout.gzipFile) {
	if (!SDDS_GZipBufferedRead(SDDS_dataset->data[i], 
				   SDDS_type_size[layout->column_definition[i].type-1]*SDDS_dataset->n_rows, 
				   gzfp, fBuffer)) {
...
#endif
        if (SDDS_dataset->layout.lzmaFile) {
          if (!SDDS_LZMABufferedRead(SDDS_dataset->data[i], 
                                     SDDS_type_size[layout->column_definition[i].type-1]*SDDS_dataset->n_rows, 
                                     lzmafp, fBuffer)) {
            SDDS_SetError("Unable to read columns--failure reading values (SDDS_ReadBinaryColumns)");
            return(0);
          ...
          if (!SDDS_BufferedRead(SDDS_dataset->data[i], 
                                 SDDS_type_size[layout->column_definition[i].type-1]*SDDS_dataset->n_rows, 
                                 fp, fBuffer)) {
            SDDS_SetError("Unable to read columns--failure reading values (SDDS_ReadBinaryColumns)");
            return(0);
...
char *SDDS_ReadNonNativeLZMABinaryString(struct lzmafile *lzmafp, SDDS_FILEBUFFER *fBuffer, int32_t skip)
{
  ...
  if (!SDDS_LZMABufferedRead(&length, sizeof(length), lzmafp, fBuffer))
    return(0);
  SDDS_SwapLong(&length);
  if (length<0)
    return(0);
    ...
    if (SDDS_dataset->layout.lzmaFile) {
      ...
      for (i=0; i<layout->n_arrays; i++) {
        ...
        for (j=0; j<layout->array_definition[i].dimensions; j++) {
          dimension = SDDS_dataset->array[i].dimension[j];
          SDDS_SwapLong(&dimension);
          if (!SDDS_LZMABufferedWrite(&dimension, sizeof(dimension),
                                      lzmafp, fBuffer)) {
            SDDS_SetError("Unable to write arrays--failure writing dimensions (SDDS_WriteNonNativeBinaryArrays)");
            SDDS_SwapEndsArrayData(SDDS_dataset);
            return(0);
